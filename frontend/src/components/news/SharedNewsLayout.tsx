...
// Self-contained time formatter so it works anywhere
...
export default function SharedNewsLayout({
  ...
  return (
    <main className="transition-colors duration-300" style={{ background:'var(--bg)', minHeight:'100vh' }}>

      {/* Category Header Banner (Only shows on Category Pages) */}
      {pageTitle && (
        <div className="py-8 px-4 transition-colors duration-300" style={{ background: 'var(--bg2)', borderBottom: '1px solid var(--border)' }}>
          <div className="max-w-7xl mx-auto">
            <nav className="text-xs mb-4" style={{ color: 'var(--muted)' }}>
              <Link href="/" className="hover:text-[var(--gold)] transition-colors">হোম</Link>
              <span className="mx-2">/</span>
              <span style={{ color: 'var(--text)' }}>{pageTitle}</span>
            </nav>
            <div className="flex items-center gap-4">
              <div className="w-1 h-10 rounded" style={{ background: 'var(--accent-red)' }} />
              <h1 className="text-3xl font-bold transition-colors" style={{ fontFamily: 'var(--font-playfair)', color: 'var(--text)' }}>
                {pageTitle}
              </h1>
            </div>
          </div>
      ...
      {/* Breaking ticker (Shows if breaking news exists) */}
      {breaking.length > 0 && (
        <div style={{ background: 'linear-gradient(90deg, #DC2626, #991B1B)', boxShadow: '0 2px 10px rgba(220,38,38,0.4)' }} className="py-2 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {articles.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-lg" style={{ color: 'var(--muted)' }}>কোনো সংবাদ পাওয়া যায়নি</p>
            <Link href="/" className="text-sm mt-4 inline-block hover:underline" style={{ color: 'var(--accent-red)' }}>হোমে ফিরে যাও</Link>
          </div>
        ) : (
          <>
            {/* Hero Section */}
            {featured && (
              <section className="mb-10">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                  ...
                  {/* Secondary Articles next to Hero */}
            ...
            {/* Main Content Grid (Latest & Trending) */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <section className="lg:col-span-2">
                <div className="section-title">{pageTitle ? `আরও ${pageTitle} সংবাদ` : 'সর্বশেষ সংবাদ'}</div>
